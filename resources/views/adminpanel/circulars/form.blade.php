...
@endif
                ...
                <div class="col-sm-2 p-0">
                    <div class="overflow-hidden">
                        <button class="btn  mx-auto mt-2" type="button" data-bs-toggle="modal"
                                data-bs-target="#addForm"  style="background-color: #1B1F23; color: #ffffff; border: none;">Add Form</button>
                    </div>
                </div>
            </div>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                @if($forms->pdf)
                    <a href="{{ asset('storage/app/public/' . $forms->pdf) }}" target="_blank" class="btn btn-primary">
                        View File
                    </a>
                @else
                    No File Available
            ...
            <td>
                ...
                <button class="btn btn-sm btn-warning formedit-btn"
        data-id="{{ $forms->id }}"
        data-name="{{ $forms->name }}"
        data-status="{{ $forms->status }}"
        data-pdf="{{ asset('storage/' . $forms->pdf) }}"
        data-bs-toggle="modal"
        data-bs-target="#editFormModal">
    <i class="fas fa-edit"></i> <!-- Edit Icon -->
</button>
<form id="delete-form-{{ $forms->id }}" action="{{ route('form.destroy', $forms->id) }}" method="POST" style="display:inline-block;">
    @csrf
    @method('DELETE')
    <button type="button" class="btn btn-sm btn-danger" onclick="confirmFormDelete({{ $forms->id }})">
...
@endif
                ...
                <div class="col-sm-2 p-0">
                    <div class="overflow-hidden">
                        <button class="btn  mx-auto mt-2" type="button" data-bs-toggle="modal"
                                data-bs-target="#addcomments"  style="background-color: #1B1F23; color: #ffffff; border: none;">Add Comments</button>
                    </div>
                </div>
            </div>
    ...
    <tbody>
        @foreach($comments as $index => $comments)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>{{ $comments->name }}</td>
  <td>{{ $comments->link }}</td>
            <td>
                @if($comments->pdf)
                    <a href="{{ asset('storage/app/public/' . $comments->pdf) }}" target="_blank" class="btn btn-primary">
                        View File
                    </a>
                @else
                    No File Available
        ...
        data-link="{{ $comments->link }}"
        data-pdf="{{ asset('storage/' . $comments->pdf) }}"
        data-bs-toggle="modal"
        data-bs-target="#editCommentsModal">
    <i class="fas fa-edit"></i> <!-- Edit Icon -->
</button>
<form id="delete-form-{{ $comments->id }}" action="{{ route('comment.destroy', $comments->id) }}" method="POST" style="display:inline-block;">
    @csrf
    @method('DELETE')
    <button type="button" class="btn btn-sm btn-danger" onclick="confirmFormDelete({{ $comments->id }})">
        <i class="fas fa-trash"></i> <!-- Delete Icon -->
    </button>
</form>
