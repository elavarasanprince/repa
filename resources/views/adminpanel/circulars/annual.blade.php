...
@endif
                ...
                <div class="col-sm-2 p-0">
                    <div class="overflow-hidden">
                        <button class="btn  mx-auto mt-2" type="button" data-bs-toggle="modal"
                                data-bs-target="#addannual"  style="background-color: #1B1F23; color: #ffffff; border: none;">Add Annual</button>
                    </div>
                </div>
            </div>
                ...
                <table id="annualTable" class="table table-striped table-bordered">
    <thead>
        <tr>
            <th>SNo.</th>
            <th>Title</th>
    ...
    <tbody>
        @foreach($annualReport as $index => $annualReport)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>{{ @$annualReport->name }}</td>
            <td>
                @if(@$annualReport->pdf)
                    <a href="{{ asset('storage/app/public/' . $annualReport->pdf) }}" target="_blank" class="btn btn-primary">
                        View File
                    </a>
                @else
                    No File Available
            ...
            <td>
                ...
                <button class="btn btn-sm btn-warning annualedit-btn"
        data-id="{{ $annualReport->id }}"
        data-name="{{ $annualReport->name }}"
        data-status="{{ $annualReport->status }}"
        data-pdf="{{ asset('storage/' . $annualReport->pdf) }}"
        data-bs-toggle="modal"
        data-bs-target="#editAnnualModal">
    <i class="fas fa-edit"></i> <!-- Edit Icon -->
</button>
<form id="delete-form-{{ $annualReport->id }}" action="{{ route('annual.destroy', $annualReport->id) }}" method="POST" style="display:inline-block;">
    @csrf
    @method('DELETE')
    <button type="button" class="btn btn-sm btn-danger" onclick="confirmAnnualDelete({{ $annualReport->id }})">
